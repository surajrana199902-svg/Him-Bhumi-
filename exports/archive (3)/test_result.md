...
agent_communication:
  - agent: "main"
    message: "Please test ONLY the new listings and verification backend endpoints in /app/app/api/[[...path]]/route.js. Do not modify frontend files. The OTP is intentionally mocked (no SMS provider) and returns devOtp in the send response so the flow can be verified end-to-end. Verify: (1) verify/send returns devOtp; verify/check succeeds with correct code and fails with wrong code;